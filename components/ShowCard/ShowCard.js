...
export default function ShowCard(props) {
  ...
  const bgImageStyles = {'background-image': `url("${imageSrc}")`};
  ...
  return (
    <li key={props.showNum} className="flex flex-col rounded-md shadow-md hover:-translate-x-1 hover:-translate-y-1 transition-all duration-200 hover:shadow-lg">
      <div className="relative w-full" style={{'padding-bottom': '140.4761905%'}}>
      ...
      <div className="p-4 flex flex-col grow w-full">
        <div className="flex flex-col grow">
          ...
          <p className="text-sm">{props.result.show?.network?.name && <h4>{props.result.show.network.name}</h4>}</p>
